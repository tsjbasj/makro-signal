...
function Nav() {
  return (
    <nav style={{ display: 'flex', alignItems: 'center', gap: 28, padding: '10px 24px', background: 'rgba(7,9,15,0.97)', borderBottom: '1px solid rgba(0,0,0,0.05)', fontFamily: 'var(--font-dm-mono)', fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', position: 'sticky', top: 0, zIndex: 100 }}>
      ...
      <Link href="/radar" style={{ color: '#1e293b', textDecoration: 'none', borderBottom: '1px solid #f59e0b', paddingBottom: 2 }}>Aktie Radar</Link>
    </nav>
  )
}
...
function StockModal({
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.72)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#f8f7f4', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: 28, width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
        {/* Header */}
          ...
          <div style={{ marginBottom: 20, padding: 16, background: 'rgba(0,0,0,0.03)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                onClick={runAnalysis}
                disabled={analysing}
                style={{ background: 'rgba(99,102,241,0.2)', border: '1px solid rgba(99,102,241,0.3)', color: analysing ? '#9ca3af' : '#a78bfa', borderRadius: 6, padding: '8px 16px', fontFamily: 'var(--font-dm-mono)', fontSize: 11, cursor: analysing ? 'wait' : 'pointer', whiteSpace: 'nowrap' }}
              >
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ background: 'none', border: '1px solid rgba(0,0,0,0.08)', color: '#9ca3af', borderRadius: 6, padding: '10px 20px', fontFamily: 'var(--font-dm-mono)', fontSize: 11, cursor: 'pointer' }}>
            Annuller
          </button>
          <button onClick={handleSave} style={{ background: 'rgba(99,102,241,0.25)', border: '1px solid rgba(99,102,241,0.4)', color: '#a78bfa', borderRadius: 6, padding: '10px 20px', fontFamily: 'var(--font-dm-mono)', fontSize: 11, cursor: 'pointer' }}>
            Gem aktie
          </button>
        </div>
      </div>
    ...
    <div style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ padding: '14px 16px 16px' }}>
        ...
        <div style={{ position: 'absolute', top: 14, right: 12, display: 'flex', gap: 6 }}>
          <button onClick={onEdit} title="Rediger" style={{ background: 'none', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: 13, padding: 2, lineHeight: 1 }}>✏</button>
          <button onClick={onDelete} title="Slet" style={{ background: 'none', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: 13, padding: 2, lineHeight: 1 }}>✕</button>
        </div>

        {/* Ticker + name */}
        <div style={{ marginBottom: 10, paddingRight: 48 }}>
          <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 16, fontWeight: 500, color: '#1e293b' }}>{stock.ticker}</span>
          <div style={{ fontFamily: 'var(--font-cormorant)', fontSize: 14, color: '#6b7280', marginTop: 2 }}>{stock.name}</div>
        </div>
        ...
        <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 10, display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 12 }}>
          <div>
            <span style={{ color: '#9ca3af' }}>Tidshorisont: </span>
            <span style={{ color: '#4a5568' }}>{stock.horizonText}</span>
          </div>
          <div>
            <span style={{ color: '#9ca3af' }}>Stop loss: </span>
            <span style={{ color: '#ef4444' }}>${stock.stopLoss}</span>
            <span style={{ color: '#9ca3af' }}> — sælg hvis kursen falder hertil</span>
          </div>
          <div>
            <span style={{ color: '#9ca3af' }}>Exit mål: </span>
            <span style={{ color: '#22c55e' }}>${stock.exitTarget}</span>
            <span style={{ color: '#9ca3af' }}> — sælg når kursen når hertil</span>
          </div>
          <div>
            <span style={{ color: '#9ca3af' }}>Tilføjet: </span>
            <span style={{ color: '#9ca3af' }}>{stock.addedDate}</span>
          </div>
        </div>

        {/* Reason */}
        <p style={{ fontFamily: 'var(--font-cormorant)', fontStyle: 'italic', fontSize: 13, color: '#6b7280', lineHeight: 1.65, margin: 0 }}>
          {stock.reason}
        </p>
      </div>
    </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8f7f4' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px 0' }}>
        ...
        <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 11, color: '#9ca3af', marginBottom: 36 }}>
            ...
            return (
              <div key={col.key}>
                ...
                <div style={{ borderTop: '2px solid ' + col.accent, paddingTop: 14, marginBottom: 16 }}>
                  <div style={{ fontFamily: 'var(--font-cormorant)', fontSize: 20, fontWeight: 600, color: '#1e293b', marginBottom: 4 }}>
                    {col.label}
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 10, color: '#9ca3af' }}>{col.sub}</span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {colStocks.length === 0 ? (
                    <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 11, color: '#9ca3af', textAlign: 'center', padding: '28px 0', border: '1px dashed rgba(255,255,255,0.06)', borderRadius: 8 }}>
                      Ingen aktier endnu
                    </div>
                  ) : (
                    colStocks.map(s => (
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '20px 24px', marginTop: 16 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, padding: 16 }}>
              ...
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: c.active ? '#22c55e' : '#9ca3af', flexShrink: 0 }} />
                  <span style={{ fontSize: 9, color: c.active ? '#4a5568' : '#9ca3af', flex: 1 }}>{c.label}</span>
                  <span style={{ fontSize: 9, color: c.active ? '#22c55e' : '#9ca3af' }}>{c.value}</span>
                </div>
              ))}
              <div style={{ fontSize: 9, color: '#9ca3af', marginTop: 8 }}>Min. 2 af 3 skal være opfyldt</div>
            </div>
          ...
          <div style={{ background: 'rgba(34,197,94,0.05)', border: '1px solid rgba(34,197,94,0.15)', borderRadius: 8, padding: '10px 14px', marginBottom: 14 }}>
            ...
            <div style={{ fontSize: 10, color: '#4a5568' }}>NU + DLO købt som rotationskøb under Extreme Fear (Fear & Greed = 16)</div>
          </div>
...
{/* Footer */}
        <div style={{ textAlign: 'center', fontFamily: 'var(--font-dm-mono)', fontSize: 9, color: '#9ca3af', letterSpacing: '0.06em', paddingBottom: 40 }}>
