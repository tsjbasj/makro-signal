...
const DOM_CONFIG = {
  KØB: { bg: 'rgba(34,197,94,0.15)', text: '#2d6a3f', border: 'rgba(34,197,94,0.3)', emoji: '✅' },
  AFVENT: { bg: 'rgba(245,158,11,0.15)', text: '#8a6a00', border: 'rgba(245,158,11,0.3)', emoji: '⏳' },
  SKIP: { bg: 'rgba(239,68,68,0.15)', text: '#8b1c1c', border: 'rgba(239,68,68,0.3)', emoji: '❌' },
}

const HORIZON_ACCENT = { kort: '#8a6a00', mellem: '#111111', lang: '#2d6a3f' }
...
function Nav() {
  return (
    <nav style={{ display: 'flex', alignItems: 'center', gap: 28, padding: '10px 24px', background: 'rgba(7,9,15,0.97)', borderBottom: '1px solid rgba(0,0,0,0.05)', fontFamily: 'var(--font-dm-mono)', fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', position: 'sticky', top: 0, zIndex: 100 }}>
      ...
      <Link href="/radar" style={{ color: '#111111', textDecoration: 'none', borderBottom: '1px solid #8a6a00', paddingBottom: 2 }}>Aktie Radar</Link>
    </nav>
  )
}
...
function StockModal({
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.72)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#f2efe6', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: 28, width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <span style={{ fontFamily: 'var(--font-cormorant)', fontSize: 22, fontWeight: 600 }}>
            {isEdit ? 'Rediger aktie' : 'Tilføj aktie'}
          </span>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#999999', cursor: 'pointer', fontSize: 20, lineHeight: 1 }}>✕</button>
        </div>

        {/* AI search — only for new stocks */}
        {!isEdit && (
          <div style={{ marginBottom: 20, padding: 16, background: 'rgba(0,0,0,0.03)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                onClick={runAnalysis}
                disabled={analysing}
                style={{ background: 'rgba(0,0,0,0.10)', border: '1px solid rgba(0,0,0,0.15)', color: analysing ? '#999999' : '#666666', borderRadius: 6, padding: '8px 16px', fontFamily: 'var(--font-dm-mono)', fontSize: 11, cursor: analysing ? 'wait' : 'pointer', whiteSpace: 'nowrap' }}
              >
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ background: 'none', border: '1px solid rgba(0,0,0,0.08)', color: '#999999', borderRadius: 6, padding: '10px 20px', fontFamily: 'var(--font-dm-mono)', fontSize: 11, cursor: 'pointer' }}>
            Annuller
          </button>
          <button onClick={handleSave} style={{ background: 'rgba(0,0,0,0.12)', border: '1px solid rgba(0,0,0,0.18)', color: '#666666', borderRadius: 6, padding: '10px 20px', fontFamily: 'var(--font-dm-mono)', fontSize: 11, cursor: 'pointer' }}>
            Gem aktie
          </button>
        </div>
      </div>
...
function StockCard({
  ...
  return (
    <div style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, overflow: 'hidden', position: 'relative' }}>
      {/* Accent line */}
      ...
      <div style={{ padding: '14px 16px 16px' }}>
        ...
        <div style={{ position: 'absolute', top: 14, right: 12, display: 'flex', gap: 6 }}>
          <button onClick={onEdit} title="Rediger" style={{ background: 'none', border: 'none', color: '#999999', cursor: 'pointer', fontSize: 13, padding: 2, lineHeight: 1 }}>✏</button>
          <button onClick={onDelete} title="Slet" style={{ background: 'none', border: 'none', color: '#999999', cursor: 'pointer', fontSize: 13, padding: 2, lineHeight: 1 }}>✕</button>
        </div>

        {/* Ticker + name */}
        <div style={{ marginBottom: 10, paddingRight: 48 }}>
          <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 16, fontWeight: 500, color: '#111111' }}>{stock.ticker}</span>
          <div style={{ fontFamily: 'var(--font-cormorant)', fontSize: 14, color: '#777777', marginTop: 2 }}>{stock.name}</div>
        </div>
        ...
        <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 10, display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 12 }}>
          <div>
            <span style={{ color: '#999999' }}>Tidshorisont: </span>
            <span style={{ color: '#555555' }}>{stock.horizonText}</span>
          </div>
          <div>
            <span style={{ color: '#999999' }}>Stop loss: </span>
            <span style={{ color: '#8b1c1c' }}>${stock.stopLoss}</span>
            <span style={{ color: '#999999' }}> — sælg hvis kursen falder hertil</span>
          </div>
          <div>
            <span style={{ color: '#999999' }}>Exit mål: </span>
            <span style={{ color: '#2d6a3f' }}>${stock.exitTarget}</span>
            <span style={{ color: '#999999' }}> — sælg når kursen når hertil</span>
          </div>
          <div>
            <span style={{ color: '#999999' }}>Tilføjet: </span>
            <span style={{ color: '#999999' }}>{stock.addedDate}</span>
          </div>
        </div>

        {/* Reason */}
        <p style={{ fontFamily: 'var(--font-cormorant)', fontStyle: 'italic', fontSize: 13, color: '#777777', lineHeight: 1.65, margin: 0 }}>
          {stock.reason}
        </p>
      </div>
    </div>
...
export default function RadarPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f2efe6' }}>
      <Nav />

      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px 0' }}>
        {/* Page header */}
        <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 10, color: '#8a6a00', letterSpacing: '0.12em', marginBottom: 8 }}>
          ◈ AKTIE RADAR
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12, marginBottom: 6 }}>
          ...
          <button
            onClick={openAdd}
            style={{ background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(0,0,0,0.15)', color: '#666666', borderRadius: 8, padding: '9px 18px', fontFamily: 'var(--font-dm-mono)', fontSize: 11, cursor: 'pointer', letterSpacing: '0.06em' }}
          >
            ＋ Tilføj aktie
          </button>
        </div>
        <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 11, color: '#999999', marginBottom: 36 }}>
            ...
            return (
              <div key={col.key}>
                ...
                <div style={{ borderTop: '2px solid ' + col.accent, paddingTop: 14, marginBottom: 16 }}>
                  <div style={{ fontFamily: 'var(--font-cormorant)', fontSize: 20, fontWeight: 600, color: '#111111', marginBottom: 4 }}>
                    {col.label}
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 10, color: '#999999' }}>{col.sub}</span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {colStocks.length === 0 ? (
                    <div style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 11, color: '#999999', textAlign: 'center', padding: '28px 0', border: '1px dashed rgba(255,255,255,0.06)', borderRadius: 8 }}>
                      Ingen aktier endnu
                    </div>
                  ) : (
                    colStocks.map(s => (
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '20px 24px', marginTop: 16 }}>
          <div style={{ fontSize: 10, color: '#111111', letterSpacing: '0.1em', marginBottom: 16 }}>ROTATIONSREGEL</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, padding: 16 }}>
              <div style={{ fontSize: 9, color: '#2d6a3f', letterSpacing: '0.08em', marginBottom: 10 }}>↓ ROTÉR IND I VÆKST</div>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: c.active ? '#2d6a3f' : '#999999', flexShrink: 0 }} />
                  <span style={{ fontSize: 9, color: c.active ? '#555555' : '#999999', flex: 1 }}>{c.label}</span>
                  <span style={{ fontSize: 9, color: c.active ? '#2d6a3f' : '#999999' }}>{c.value}</span>
                </div>
              ))}
              <div style={{ fontSize: 9, color: '#999999', marginTop: 8 }}>Min. 2 af 3 skal være opfyldt</div>
            </div>

            {/* Rotér UD */}
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, padding: 16 }}>
              <div style={{ fontSize: 9, color: '#8b1c1c', letterSpacing: '0.08em', marginBottom: 10 }}>↑ ROTÉR UD AF VÆKST</div>
              {[
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
            {[
              { label: 'Rotationer brugt i år', value: '1 af 3', color: '#2d6a3f' },
              { label: 'Sidst roteret', value: 'Apr 2026', color: '#555555' },
              { label: 'Cooldown opfyldt', value: 'Nej (+15pt mangler)', color: '#8a6a00' },
              { label: 'Max pr. rotation', value: '2 aktier', color: '#777777' },
            ].map((s, i) => (
              <div key={i} style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 6, padding: '6px 12px' }}>
                <div style={{ fontSize: 8, color: '#999999', marginBottom: 2 }}>{s.label}</div>
          ...
          <div style={{ background: 'rgba(34,197,94,0.05)', border: '1px solid rgba(34,197,94,0.15)', borderRadius: 8, padding: '10px 14px', marginBottom: 14 }}>
            <div style={{ fontSize: 9, color: '#2d6a3f', marginBottom: 4 }}>◈ AKTUEL ROTATION — APRIL 2026</div>
            <div style={{ fontSize: 10, color: '#555555' }}>NU + DLO købt som rotationskøb under Extreme Fear (Fear & Greed = 16)</div>
          </div>

          {/* Regler */}
          <div style={{ fontSize: 9, color: '#999999', lineHeight: 1.7 }}>
            {[
...
{/* Footer */}
        <div style={{ textAlign: 'center', fontFamily: 'var(--font-dm-mono)', fontSize: 9, color: '#999999', letterSpacing: '0.06em', paddingBottom: 40 }}>
